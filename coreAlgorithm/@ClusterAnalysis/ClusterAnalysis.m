    ...
    methods
        ...
        function obj = ClusterAnalysis(localizationFile, varargin)
            ...
            switch randomAlgorithm
                case 'averageDensity'
                    if obj.dimension == 3
                        obj.localDensity = obj.NoOfPoints / (obj.physicalDimension(1, 1)/1000 * obj.physicalDimension(2, 1)/1000 * obj.physicalDimension(3, 1)/1000); % in �m^3
                    else
                        obj.localDensity = obj.NoOfPoints / (obj.physicalDimension(1, 1)/1000 * obj.physicalDimension(2, 1)/1000); % in �m^2
                    end
                case 'kernelDensity'
                    ...
                    if obj.dimension == 3
                        ...
                        % transform KDEestimate into density per �m�
                        ...
                        % transform z from nm into density per �m�
                        x = linspace(min(x1), max(x1), xGridSpace);
                        y = linspace(min(x2), max(x2), yGridSpace);
            ...
            if assumeCSR == true
                ...
                if obj.dimension == 3
                    nPoints = poissrnd(obj.localDensity*obj.physicalDimension(1, 1)/1000*obj.physicalDimension(2, 1)/1000*obj.physicalDimension(3, 1)/1000); % 1000 because localDensity is in �m^3 
                else
                    nPoints = poissrnd(obj.localDensity*obj.physicalDimension(1, 1)/1000*obj.physicalDimension(2, 1)/1000); % 1000 because localDensity is in �m^2 
                end
        ...
        [varargout] = voronoiCluster( obj, isRandom, dim, showImage, saveImageFolder, saveImageFilename, max_area, max_cum_area)
