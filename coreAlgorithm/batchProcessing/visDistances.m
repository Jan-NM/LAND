function visDistances(Clusterparamstruct, clusterData)
...
if Clusterparamstruct.Distance.algorithm == true
	...
	for kk=1:nSamples
		if isfield(clusterData(kk).Analysis.clusterStruct, 'allDistances')
			% for cell2mat conversion, every cell should have the same data
			% type -> conversion to single
			totalDistance{kk} = single(clusterData(kk).Analysis.clusterStruct(1).allDistances);
		else
			totalDistance{kk} = single(0);
		end
	end
	...
	if Clusterparamstruct.compareRandomData == true
		for kk=1:nSamples
			if isfield(clusterData(kk).Analysis.randomClusterStruct, 'allDistances')
				% for cell2mat conversion, every cell should have the same data
				% type -> conversion to single
				totalDistanceRandom{kk} = single(clusterData(kk).Analysis.randomClusterStruct(1).allDistances);
			else
				totalDistanceRandom{kk} = single(0);
			end
		end
